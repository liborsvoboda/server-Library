...
; (function ($, window, document, undefined) {
    ...
    Waterfall.prototype = {
        ...
        clear: function () {
            // 移除已有元素
            var $content = this.$element.find('.' + this.options.itemCls);
            var $items = this.options.isFadeIn ? this._getItems($content).css({ opacity: 0 }).animate({ opacity: 1 }) : this._getItems($content);
            this.removeItems($items);
            // 移除错误信息
            this.$message.html('');
            // 移除loading信息
            var options = this.options;
            options.callbacks.loadingFinished(this.$loading, false);
        },
        ...
        refresh: function (opts) {
            ...
            // reset status
            options.state.isResizing = false;
            options.state.isProcessingData = false;
            options.state.isDuringAjax = false;
